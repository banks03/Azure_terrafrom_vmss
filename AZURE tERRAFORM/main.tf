...
resource "azurerm_public_ip" "pip" {
  ...
  name                = lower("pip-vm-${var.vmscaleset_name}-${data.azurerm_resource_group.assessment.location}-0${count.index + 1}")
  location            = data.azurerm_resource_group.assessment.location
  resource_group_name = data.azurerm_resource_group.assessment.name
...
resource "azurerm_lb" "vmsslb" {
  ...
  name                = var.load_balancer_type == "public" ? lower("lbext-${var.vmscaleset_name}-${data.azurerm_resource_group.assessment.location}") : lower("lbint-${var.vmscaleset_name}-${data.azurerm_resource_group.assessment.location}")
  location            = data.azurerm_resource_group.assessment.location
  resource_group_name = data.azurerm_resource_group.assessment.name
...
resource "azurerm_lb_nat_pool" "natpol" {
  ...
  name                           = lower("lbe-nat-pool-${var.vmscaleset_name}-${data.azurerm_resource_group.assessment.location}")
  resource_group_name            = data.azurerm_resource_group.assessment.name
...
resource "azurerm_proximity_placement_group" "appgrp" {
  ...
  name                = lower("proxigrp-${var.vmscaleset_name}-${data.azurerm_resource_group.assessment.location}")
  resource_group_name = data.azurerm_resource_group.assessment.name
  location            = data.azurerm_resource_group.assessment.location
}
...
resource "azurerm_network_security_group" "nsg" {
  ...
  name                = lower("nsg_${var.vmscaleset_name}_${data.azurerm_resource_group.assessment.location}_in")
  resource_group_name = data.azurerm_resource_group.assessment.name
  location            = data.azurerm_resource_group.assessment.location
}
...
resource "azurerm_linux_virtual_machine_scale_set" "linux_vmss" {
  name                                              = format("vm%s%s", lower(replace(var.vmscaleset_name, "/[[:^alnum:]]/", "")), 1)
  computer_name_prefix                              = var.computer_name_prefix == null && var.instances_count == 1 ? substr(var.vmscaleset_name, 0, 15) : substr(format("%s%s", lower(replace(var.vmscaleset_name, "/[[:^alnum:]]/", "")), 1), 0, 15)
  resource_group_name                               = data.azurerm_resource_group.assessment.name
  location                                          = data.azurerm_resource_group.assessment.location
  ...
  network_interface {
    name                          = lower("nic-${format("vm%s%s", lower(replace(var.vmscaleset_name, "/[[:^alnum:]]/", "")), 1)}")
    primary                       = true
    ...
    ip_configuration {
      name                                   = lower("ipconig-${format("vm%s%s", lower(replace(var.vmscaleset_name, "/[[:^alnum:]]/", "")), 1)}")
      primary                                = true
      ...
      dynamic "public_ip_address" {
        ...
        content {
          name                = lower("pip-${format("vm%s%s", lower(replace(var.vmscaleset_name, "/[[:^alnum:]]/", "")), "0${1}")}")
  ...
  dynamic "boot_diagnostics" {
    ...
    content {
      storage_account_uri = var.storage_account_name != null ? azurerm_storage_account.storeacc.primary_blob_endpoint : var.storage_account_uri
    }
  }
}
...
resource "azurerm_log_analytics_workspace" "workspace" {
  name                = lower("${var.vmscaleset_name}-log-analytics-workspace")
  location            = data.azurerm_resource_group.assessment.location
  resource_group_name = data.azurerm_resource_group.assessment.name
...
resource "azurerm_virtual_machine_scale_set_extension" "omsagentlinux" {
  count                        = var.deploy_log_analytics_agent ? 1 : 0
...
resource "azurerm_monitor_diagnostic_setting" "nsg" {
  count                      = var.existing_network_security_group_id == null != null ? 1 : 0
...
resource "azurerm_monitor_diagnostic_setting" "lb-pip" {
  count                      = var.load_balancer_type == "public" != null ? 1 : 0
...
resource "azurerm_monitor_diagnostic_setting" "lb" {
  count                      = var.load_balancer_type == "public" && var.storage_account_name != null ? 1 : 0
